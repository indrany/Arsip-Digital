            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" style="font-size: 13px;">
                    <thead class="bg-light">
                        <tr>
                            <th class="ps-4">No. Permohonan</th>
                            <th>Nama Pemohon</th>
                            <th>Asal Unit</th>
                            <th>Tanggal Kirim</th>
                            <th>Status</th>
                    ...
                    <tbody>
                        @forelse($riwayat_batches as $row)
                        <tr>
                            <td class="ps-4 fw-bold text-primary">{{ $row->no_permohonan }}</td>
                            <td class="text-dark">{{ $row->nama }}</td>
                            <td><span class="badge bg-info text-dark px-2">{{ $row->asal_unit ?? 'Kanim' }}</span></td>
                            <td>{{ \Carbon\Carbon::parse($row->tgl_pengirim)->format('d-m-Y') }}</td>
                            <td>
                                <span class="badge rounded-pill {{ $row->status == 'DITERIMA OLEH ARSIP' ? 'bg-success' : 'bg-warning text-dark' }} px-3">
                                    {{ $row->status }}
                                </span>
                            </td>
                            <td class="text-center">
                                @if($row->status != 'DITERIMA OLEH ARSIP')
                                    <button class="btn btn-primary btn-sm btn-proses-batch" data-id="{{ $row->batch_id }}" style="border-radius: 8px;">
                                        <i class="fas fa-barcode me-1"></i> Mulai Terima Berkas
                                    </button>
                                @else
                                    <span class="text-muted small"><i class="fas fa-check-circle text-success me-1"></i>Selesai Verifikasi</span>
                                @endif
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="6" class="text-center py-5 text-muted">Tidak ada kiriman berkas masuk.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="col-lg-6">
            <div class="card border-0 shadow-sm p-4" style="min-height: 500px; border-radius: 15px;">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <span id="pending-count" class="badge bg-light text-dark border text-dark">0 Berkas</span>
                </div>
                <div class="table-responsive">
...
$(document).ready(function() {
    ...
    function updateButtonStatus() {
        ...
        // Jika sisa sudah 0, aktifkan tombol Selesaikan
        ...
        $.get(`/arsip/list-berkas/${currentBatch}`, function(res) {
            if(res.success) {
                ...
                res.data.forEach(item => {
                    // Cek apakah berkas sudah pernah di-scan (DITERIMA) atau sudah selesai (DITERIMA OLEH ARSIP)
                    ...
                    // Tabel Kiri (Tetap tampilkan semua agar user tahu mana yang belum hijau)
                    ...
                    // Tabel Kanan (Hanya tampilkan yang statusnya sudah discan)
                    if(isAlreadyScanned) {
    ...
    inputBarcode.on('keypress', function(e) {
        if (e.which === 13) {
            ...
            $.post("{{ route('penerimaan-berkas.scan-permohonan') }}", { nomor_permohonan: barcode }, function(res) {
                ...
                playBeep('error');
                let msg = xhr.responseJSON ? xhr.responseJSON.message : 'Gagal verifikasi ke server!';
    ...
    // Pastikan fokus ke input barcode saat klik area manapun (kecuali modal)
...
// Fungsi Detail (Sama seperti sebelumnya)
