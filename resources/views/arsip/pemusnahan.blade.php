                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="col-periode">
                                        {{ \Carbon\Carbon::parse($row->filter_mulai)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($row->filter_selesai)->format('d/m/Y') }}
                                    </td>
                                    <td class="col-jumlah">
                                        <span class="badge bg-secondary-subtle text-secondary px-3">{{ $row->jumlah_dokumen }} Berkas</span>
                                    </td>
                                    <td class="col-status text-center">
                                        {{-- Pengecekan status sinkron dengan ENUM Database --}}
                                    ...
                                    <td class="col-aksi">
                                        <div class="d-flex justify-content-center gap-2">
                                            ...
                                            {{-- Aksi Khusus ADMIN (Hanya muncul jika status 'Diajukan') --}}
                                            @if(strtoupper(Auth::user()->role) == 'ADMIN' && $row->status == 'Diajukan')
                                                {{-- Form Setuju (PAKAI CENTANG) --}}
                                                <form action="{{ route('pemusnahan.approve', $row->id) }}" method="POST" class="d-inline" id="form-setuju-{{ $row->id }}">
                                                    @csrf
                                                    <button type="button" class="btn btn-primary btn-sm shadow-sm btn-konfirmasi-setuju" data-id="{{ $row->id }}" style="height: 35px; width: 35px;" title="Setujui">
                                                        <i class="fas fa-check"></i>
                                                    </button>
                                                </form>
                            
                                                {{-- Form Tolak (PAKAI SILANG) --}}
                                                <form action="{{ route('pemusnahan.reject', $row->id) }}" method="POST" class="d-inline" id="form-tolak-{{ $row->id }}">
                                                    @csrf
                                                    <button type="button" class="btn btn-danger btn-sm shadow-sm btn-konfirmasi-tolak" data-id="{{ $row->id }}" style="height: 35px; width: 35px;" title="Tolak">
                                                        ✕
                                                    </button>
                                                </form>
                                            @endif
                            
                                            {{-- Aksi Khusus TIKIM (Upload PDF) --}}
                                            @if(strtoupper(Auth::user()->role) == 'TIKIM')
                                            <button type="button" class="btn btn-action btn-info text-white shadow-sm btn-upload-langsung" data-id="{{ $row->id }}" title="Upload PDF Susulan">
                                ...
                                @empty
                                <tr><td colspan="5" class="text-center py-5 text-muted">Data riwayat tidak ditemukan</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
